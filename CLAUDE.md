...
**MVP-1 complete.** Seven domain modules, and push reminders on top of them (`notification`, below):
...
- **notification** — `POST|DELETE /v1/me/devices`, and the morning reminder job. See the paragraph below.
...
**`internal/notification` sends the push reminders, and decides nothing about what is late** (SPEC.md D-19). The job reads the alerts screen itself — `insight.Service.Alerts` through a port the module declares, adapted in `internal/app/reminder_ports.go`, because nothing imports insight — so a push can never disagree with the screen it opens. It stores two things: `push_devices` (where a reminder can go; the token is unique and moves to whoever registered it last, because it names an installation, not a person) and `notification_log` (what each person was already told; its unique key `(user_id, kind, subject_id, marker)` is the deduplication, and `marker` is the due point, so the next oil change is reminded afresh). Four rules hold it together, and each has a test in `test/integration/notification_test.go`: **once per stage** — entering "vence em breve", falling due, and the day itself for IPVA, licenciamento and seguro; **one push per car per day, at 9h in São Paulo** — the job wakes every ten minutes in-process and works only in that hour, so a deploy at 9:05 does not lose the day (`REMINDERS_HOUR` moves the hour, only to try the whole path on a phone now; delete it afterwards); **recorded before sent** — a reminder lost beats a reminder repeated; **no sender, no job** — without `FCM_SERVICE_ACCOUNT` nothing is recorded as sent, and `NOTIFICATIONS_DEBUG=true` logs instead of sending. The transport is `internal/platform/push`: FCM's HTTP v1 API with the OAuth assertion signed by the `golang-jwt` already in go.mod, not the Admin SDK. A token is pruned only when FCM says the token itself is gone (`UNREGISTERED`, or `INVALID_ARGUMENT` about the registration token) — a bare 404 or 400 can be a wrong project or a malformed message, and pruning on it would unsubscribe every phone after one bad deploy. The notification text is written here, in Go, because the phone draws it; `text.go` mirrors the app's phrases and `Vehicle.shortName`, and a change to one is a change to both. Android only: the channel id `lembretes` must equal the app's.
...
- **Three outcomes per item, and the third is the point.** Has it → a normal plan. Does not → a plan marked `not_applicable`, off every screen and still undoable. **Cannot tell → no row at all.** A row would be a claim, and we do not have one.
- **`internal/maintenance/powertrain.go` is the only automatic source of applicability**, and it only knows what a powertrain *is* — `vehicles.fuel_type` → what the car has. It must never grow to know a make, a model or a manufacturer interval; that is the universal automotive database this project is not building. The one duplicate of it is the snapshot inside migration 000010, which is deliberate: a migration has to be self-contained.
- **Belt versus chain is asked, never inferred.** Swapping the seed for `corrente_comando` would only move the wrong assumption, so neither is suggested. `internal/maintenance/profile.go` holds a fixed list of questions in Go — not a rules engine, not a DSL — and **"não sei" is a real answer**: stored in `vehicle_profile_answers`, so the question stops coming back, and deciding nothing.
...
**A record can grow a line, and can never lose one.** `POST /v1/maintenance-records/{id}/items` appends, and that is the whole of it. It exists because PATCH deliberately does not touch lines, which left a revisão saved with five items and a forgotten brake fluid with two ways out and both bad: a history that stays wrong, or a retraction and six lines typed again. Four rules, and each is what makes appending safe where replacing is not:

- **No odometer write.** The event already produced its reading when it was created; naming one more thing that was done at the same time asserts no new distance. The record's `updated_at` moves and nothing else does.
- **The new line resets its own item from this record**, exactly as if it had been there all along — `ListLastPerformedByItem` reads `maintenance_record_items` directly, so no engine change was needed for that to be true.
- **An item already on the record is a 422, not a second line.** Two lines for one item on one event would double it in every total and say nothing new.
- **A service cannot be appended to a record with no mileage.** That is a care-only record, it asserts no distance (RN-03), and the due engine needs one. Inventing it from the odometer cache is precisely the shortcut RN-03 exists to forbid; the answer is to PATCH the mileage in first.

Removing a line stays undecided and unimplemented. Retracting the record is the way to undo a wrong one, and it already answers the question this does not: the clock of every item involved falls back to the previous record.
...
- **Three tests are guards rather than tests**, and they are the reason the rest keeps working:
  ...
  - `TestRouterAndOpenAPIAgree` compares the served routes against `api/openapi.yaml`. Drift between the code and the contract is now a failing build, not a discovery. The app writes its Dart models by hand; the matching guard on that side is `test/contract/openapi_paths_test.dart`, which reads this file and fails if the app references a route that does not exist.
...
- **`api/openapi.yaml` is the contract, and it is hand-written.** Change a request or response DTO and you change it in the same edit. The Flutter app writes its Dart models by hand — no `openapi-generator`, no `json_serializable` — so an unknown enum can fall through to a safe default instead of crashing a published build. `TestRouterAndOpenAPIAgree` fails the build when a route drifts from the spec. That guard compares paths and methods, not schemas, so a changed field is still on you — the golden files are the other half, and the app's `test/contract/openapi_paths_test.dart` is the remaining one: it reads this `openapi.yaml` and fails if the app references a route that does not exist. Validate with `docker run --rm -v "$PWD:/spec" redocly/cli lint /spec/api/openapi.yaml`.
